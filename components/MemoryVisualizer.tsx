...
export default function MemoryVisualizer({ code, variableName }: MemoryVisualizerProps) {
  ...
  useEffect(() => {
    // Simple regex check for "let mut crate_id"
    const mutRegex = new RegExp(`let\\s+mut\\s+${variableName}`);
    setIsMutable(mutRegex.test(code));

    // Check for assignment if mutable
    if (isMutable) {
      ...
      <div className="flex-1 flex items-center justify-center">
        <div className="relative">
          ...
          <motion.div
            ...
            className={`w-48 h-48 border-2 rounded-2xl flex flex-col items-center justify-center relative bg-[#1A1A23] transition-colors duration-500 ${
              isMutable ? 'border-green-500' : 'border-red-500'
            }`}
          >
            {/* Lock Icon */}
            ...
            {/* Mutability Badge */}
            <div className={`absolute bottom-4 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-widest ${
              isMutable ? 'bg-green-500/10 text-green-500' : 'bg-red-500/10 text-red-500'
            }`}>
