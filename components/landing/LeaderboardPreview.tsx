...
export function LeaderboardPreview() {
  return (
    <section className="py-24 px-6 border-y border-white/5 bg-[#0A0A0F]">
      <div className="max-w-5xl mx-auto text-center">
         ...
         <div className="bg-[#151520] rounded-3xl border border-white/5 overflow-hidden">
            <div className="grid grid-cols-12 gap-4 p-4 text-xs font-bold text-gray-500 uppercase tracking-widest border-b border-white/5 bg-black/20">
            ...
            {[
              ...
              { rank: 4, name: "You?", title: "Recruit", xp: "0", color: "text-gray-600" },
            ].map((player, i) => (
              <div key={i} className={`grid grid-cols-12 gap-4 p-6 items-center hover:bg-white/5 transition-colors ${player.name === 'You?' ? 'bg-primary/10 border-l-4 border-primary' : ''}`}>
                 ...
                 <div className="col-span-6 text-left flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-full ${player.name === 'You?' ? 'bg-primary' : 'bg-gray-700'} flex items-center justify-center text-white font-bold`}>
                       {player.name[0]}
                    </div>
